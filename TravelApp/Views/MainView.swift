...
private struct SafeAreaInsetsKey: EnvironmentKey {
    static var defaultValue: EdgeInsets {
        // 1. Grab the active UIWindowScene
        let windowScene = UIApplication.shared.connectedScenes
            .first { $0.activationState == .foregroundActive }
            as? UIWindowScene

        // 2. From that scene, find the key window
        let keyWindow = windowScene?
            .windows
            .first { $0.isKeyWindow }

        // 3. Read its safeAreaInsets (UIEdgeInsets) or fall back to zero
        let insets = keyWindow?.safeAreaInsets ?? .zero

        // 4. Convert to SwiftUI EdgeInsets
        return EdgeInsets(
            top:     insets.top,
            leading: insets.left,
